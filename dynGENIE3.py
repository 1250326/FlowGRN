# Extracted from https://github.com/vahuynh/dynGENIE3/blob/master/dynGENIE3_python/dynGENIE3.py
# Forced to run random forest with all CPUs available.
...
def dynGENIE3_single(TS_data,time_points,SS_data,output_idx,alpha,input_idx,tree_method,K,ntrees,compute_quality_scores,save_models):
    ...
    if tree_method == 'RF':
        treeEstimator = RandomForestRegressor(n_estimators=ntrees,max_features=max_features,oob_score=oob_score, n_jobs=-1)
    elif tree_method == 'ET':
